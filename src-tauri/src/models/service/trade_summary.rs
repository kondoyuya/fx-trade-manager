...
impl TradeSummary {
    pub fn from_trades(trades: Vec<Trade>) -> Self {
        ...
        let avg_profit_wins = if wins > 0 { profit_wins / wins as f64 } else { 0.0 };
        let avg_profit_losses = if losses > 0 { profit_losses / losses as f64 } else { 0.0 };
        let avg_profit_pips_wins = if wins > 0 { profit_pips_wins / wins as f64 } else { 0.0 };
        let avg_profit_pips_losses = if losses > 0 { profit_pips_losses / losses as f64 } else { 0.0 };
        let avg_holding_time = if count > 0 { total_holding_time / count as f64 } else { 0.0 };
        let avg_holding_time_wins = if wins > 0 { total_holding_time_wins / wins as f64 } else { 0.0 };
        let avg_holding_time_losses = if losses > 0 { total_holding_time_losses / losses as f64 } else { 0.0 };
