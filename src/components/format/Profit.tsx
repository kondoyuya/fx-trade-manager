...
export const Profit: React.FC<ProfitProps> = ({ profit, className = "px-2 py-1 text-right font-semibold", toFix = 0, displayMode = ""}) => {
...
export const formatProfit = (
  ...
  const value = displayMode === "円"
    ? (yen ?? 0)
    : (pips ?? 0) / 10;

  if (withUnit) {
    return <Profit profit={value} toFix={displayMode === "円" ? 0 : 1} displayMode={displayMode}/>
  } else {
